"""
Script de classification de textes par DÉCENNIES (1950s, 1960s, etc.)
...
def charger_donnees(chemin_dossier):
    ...
    Lit le contenu de chaque fichier .txt et extrait l'étiquette (la décennie)
...
# Parcours des dossiers par ère (1950-1975, 1976-1999, etc.)
    for dossier_era in chemin_base.iterdir():
        if not dossier_era.is_dir():
            continue
        
        # La classe est obtenue à partir du nom du dossier (l'ère)
        classe = dossier_era.name  

        # Parcours des fichiers texte dans ce dossier d'ère
        for fichier_txt in dossier_era.glob("*.txt"):
            try:
...
def main():
    ...
    modeles = {
        "Naïf Bayésien (MultinomialNB)": MultinomialNB(fit_prior=False),
